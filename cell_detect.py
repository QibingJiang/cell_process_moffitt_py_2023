...
# I don't have to use annulus to detect cells. So I thought maybe I just use 2 overlap contours to detect cells. Actually the performance is not good.
    ...
    def detect_by_edge_core_and_level_RFP(self, path, frame, frame_index, scale):
        ...
        bgr = cv2.imread("/home/qibing/Work/ground_truth/RFP/t" + "{0:0=3d}".format(frame_index) + ".tif")
        # print("enter detect_and_level")
        ...
        for i in range(len(contours)):
            try:
                ...
                if (max(1, self.radius_thr[0]) * scale < radius and frame_0[y][x] == 255): # < self.radius_thr[1] * scale
                    ...
                    if(cell_sum > 100000):
                        level = 0
                        pixel = bgr[int(centeroid[1] / scale)][int(centeroid[0] / scale)]
                        red = max(0, float(pixel[2]) - mean(pixel[0:2]))

                        # if(red > 200):
                        #     print(int(centeroid[1] / scale), int(centeroid[0] / scale), pixel, red)
                        #     cv2.namedWindow('bgr', cv2.WINDOW_NORMAL)
                        #     cv2.resizeWindow('bgr', 900, 900)
                        #     cv2.imshow('bgr', bgr)
                        #     cv2.waitKey()


                        b = np.array([centeroid[0] / scale, centeroid[1] / scale, ratio, area, level, 0, contours[i], box, radius, frame_index, bright_mean, eccentricity, red], dtype=object)
                        very_white_cell.append(b)
                    else:
                        level = 1
                        pixel = bgr[int(centeroid[1] / scale)][int(centeroid[0] / scale)]

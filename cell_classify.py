...
#         #         print(tracks[tra_i].coordinates)
    ...
    def analyse_classification_8(self, outpath, frame_count, gt_video_path, scale, Beacon, gt = False):
        ...
        for tracks in (self.tracks, self.del_tracks):
            for idx in range(len(tracks)):
                ...
                # if(tracks[idx].track_id == 20):
                #     print(tracks[idx].track_id)
                #     pass
    ...
    #The version 9 is to prepare data for moffitt with red fluo
    def analyse_classification_9(self, outpath, frame_count, gt_video_path, scale, Beacon, gt = False):
        # print("tracker save.")
        self.image_amount = frame_count
        window_radius = 4
        ...
        file.write("track_id, image_index, cell_diff, cell_diff_max_min, cell_diff_max_min_der, area, area_diff, area_diff_max_min, area_diff_max_min_der, cell_death, red_fluo\n")
        ...
        for tracks in (self.tracks, self.del_tracks):
            for tra_i in range(0, len(tracks), 1):
                ...
                if (file):
                    track_mat = np.zeros((self.image_amount, 11), dtype = float)
                    track_mat[:,0] = tracks[tra_i].track_id
                    track_mat[:,1] = np.arange(self.image_amount)
                    track_mat[:,2] = tracks[tra_i].cell_diff[:self.image_amount]
                #     file.write("track: %d;\n" % tracks[tra_i].track_id)
                ...
                for img_idx in range(self.image_amount):
                    if(tracks[tra_i].full_trace[img_idx] != None):
                        track_mat[img_idx][10] = tracks[tra_i].full_trace[img_idx][12]
                    else:
                        track_mat[img_idx][10] = np.nan
